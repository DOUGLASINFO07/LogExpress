...
<table width="487" height="583" border="0">
  ...
  <tr>
    ...
    <?php
		...
		$notaFiscal = $_POST["txtnotaFiscal"];
	 ...
	 if ($camposOK){
		...
		while ($dados = mysql_fetch_assoc($res)){
				...
				$sql = "UPDATE ordemEntrega SET observacao = '$atualizacao'  where notaFiscal = '$notaFiscal' "; 
		
		$res = mysql_query ($sql) or die ("ERRO ao Atualizar dados Ordem Entrega.".mysql_error());
		
					
						
						/*"NOME DA EMPRESA = $nomeCliente <br>
		...
		<H2>ATUALIZAR DADOS DE RASTREAMENTO DE MERCADORIA</H2><BR />
		
		<form id='form1' name='form1' method='post' 
		action='http://localhost/Site/Atualizado.php'>
			  
		<em><strong>OBSERVACOES</strong></em><br>
		
<input name='txtobservacao' type='text'/>
